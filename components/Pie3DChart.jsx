        ...
        <div ref={wrapRef} style={{ width: "100%", height, position: "relative" }}>
            ...
            {tooltip && (
                <div
                    ...
                    style={{
                        left: tooltip.x,
                        top: tooltip.y - (tooltip.subs?.length > 0 ? 24 + tooltip.subs.length * 28 + 64 : 68),
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between gap-3 px-4 py-2.5"
                            style={{ borderBottom: tooltip.subs?.length > 0 ? "1px solid rgba(128,128,128,0.15)" : undefined }}
                        >
                            <div className="flex items-center gap-2 min-w-0">
                                ...
                                />
                                <span className="text-sm font-semibold text-foreground truncate">{tooltip.name}</span>
                            </div>
                            <div className="text-right shrink-0">
                                <span className="text-sm font-bold text-foreground">{tooltip.value.toLocaleString()}</span>
                                <span className="text-xs text-muted-foreground ml-1">({tooltip.pct.toFixed(1)}%)</span>
                            </div>
                        </div>
                        ...
                        {tooltip.subs?.length > 0 ? (() => {
                                ...
                                <div className="px-4 pt-2 pb-3 space-y-2">
                                    ...
                                    {tooltip.subs.map((sub) => (
                                        <div key={sub.name} className="flex items-center gap-2">
                                            <span className="text-xs text-muted-foreground truncate flex-1 min-w-0">{sub.name}</span>
                                            <span className="text-xs font-semibold tabular-nums shrink-0" style={{ color: tooltip.fill }}>
                                                {sub.value}
                                            </span>
                                            <div className="w-14 h-1.5 rounded-full bg-muted shrink-0 overflow-hidden">
                                                <div
                                                    className="h-full rounded-full"
                                                    style={{ width: `${(sub.value / maxVal) * 100}%`, backgroundColor: tooltip.fill, opacity: 0.8 }}
                                                />
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="px-4 pt-1.5 pb-2.5">
                                <p className="text-[10px] text-muted-foreground/50 italic">No subconcern data available</p>
                            </div>
                        )}
                    </div>

                    {/* Caret pointing down toward the pie */}
                    <div style={{
                        width: 0,
                        height: 0,
